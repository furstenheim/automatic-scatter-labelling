...
function rayIntersection (pointsToLabel, pointsNotToLabel) {
  ...
  while (P.length !== 0) {
    ...
    let vi = {x: rij.vector.x * rij.available.getMin(), y: rij.vector.y * rij.avaiable.getMin()}
    extendedPointMethods.promoteLabelToRectangle(pi, vi)
    let index = pointsToLabel.findIndex(el => el === pi)
    P0.splice(index)
    pointsLabeled.push(P.splice(index)[0])
    for (let pk of P0) {
      ...
      if (pk.availableMeasure === 0 && P.findIndex(el => el === pk)){
