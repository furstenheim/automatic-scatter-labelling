...
/***/ function(module, exports, __webpack_require__) {
	...
	let render = (() => {
	  var _ref = _asyncToGenerator(function* (data, xAxis, yAxis) {
	    const radius = 3.5;
	    x.domain(d3.extent(data, function (d) {
	      return d.obesity_percentage;
	    })).nice();
	    y.domain(d3.extent(data, function (d) {
	      return d.life_expectancy_at_60;
	    })).nice();
	    //data = _.filter(data, d => _.includes(['Ireland', 'United Kingdom', 'Luxembourg'], d.country))

	    const labels = svg.selectAll('text.graph-label').data(data, function (d, i) {
	      return d.id;
	    }).enter().append('text').attr('class', 'graph-label').attr('font-size', '20px').attr('text-anchor', 'middle').attr('x', function (d) {
	      return x(d.obesity_percentage);
	    }).attr('y', function (d) {
	      return y(d.life_expectancy_at_60);
	    }).attr('opacity', 100).text(function (d) {
	      return d.country;
	    }).each(function (d) {
	      // Get sizes for the algorithm
	    ...
	    const extendedPoints = data.map(function (d) {
	      return {
	        id: d.id,
	        position: {
	    ...
	    //debugger
	    var a = new Date();
	    const result = yield mainAlgorithm(extendedPoints, { MAX_NUMBER_OF_ITERATIONS: 20, isWebgl: false, NUMBER_OF_RAYS: 12, radius: 3 * radius, bbox: { top: -margin.top, bottom: -margin.top - height, left: margin.left, right: margin.left + width, width, height: height } });
	    console.log('index.js:81:14:(new Date() -a) / 1000', (new Date() - a) / 1000); // 40s
	    //console.log(result)
	    const dots = svg.selectAll('.dot').data(data);
	    dots.enter().append('circle').attr('class', 'dot').attr('r', radius).attr('opacity', 100).attr('cx', function (d) {
	      return x(d.obesity_percentage);
	    }).attr('cy', function (d) {
	      return y(d.life_expectancy_at_60);
	    }).style('fill', function (d) {
	      return color(d.development_group);
	    });
	    dots.exit().transition().duration(500).attr('opacity', 0).attr('r', 0).remove();

	    const lines = svg.selectAll('.segment').data(result);
	    lines.enter().append('line').attr('class', 'segment').style('stroke', 'black').style('stroke-with', '2px').attr('x1', function (d) {
	      return idToPoints[d.id][0].position.x;
	    }).attr('y1', function (d) {
	      return -idToPoints[d.id][0].position.y;
	    }).attr('x2', function (d) {
	      return (d.rectangle.left + d.rectangle.right) / 2;
	    }).attr('y2', function (d) {
	      return -(d.rectangle.top + d.rectangle.bottom) / 2;
	    });

	    const label2 = svg.selectAll('text.graph-label').data(result, function (d, i) {
	      return d.id;
	    }); // id corresponds to previous position

	    label2.transition().duration(500).attr('opacity', 100).attr('x', function (d) {
	      return (d.rectangle.left + d.rectangle.right) / 2;
	    }).attr('y', function (d) {
	      return -(d.rectangle.top + d.rectangle.bottom) / 2;
	    });
	    label2.exit().each(function (d, i) {
	      //console.log(d, i)
	    }).remove();
	...
	const mainAlgorithm = __webpack_require__(2).mainAlgorithm;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var MainAlgorithmWorker = __webpack_require__(15);
	const webgl = __webpack_require__(12);
...
/***/ function(module, exports, __webpack_require__) {
	...
	const mainIntersectionFragment = __webpack_require__(6);
	const segmentSegmentIntersectsFragment = __webpack_require__(10);
	const segmentSegmentIntersectionFragment = __webpack_require__(9);
	const labelRectangleIntersectionFragment = __webpack_require__(3);
	const labelSegmentIntersectionFragment = __webpack_require__(4);
	const rayRectangleIntersectionFragment = __webpack_require__(7);
	const raySegmentIntersectionFragment = __webpack_require__(8);
...
/***/ function(module, exports, __webpack_require__) {
	...
	const mainFragment = __webpack_require__(5).mainFragment;
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = function() {
		return new Worker(__webpack_require__.p + "69a2efb15aaeee820ba0.worker.js");
	};
