...
let findBestRay = (() => {
  var _ref = _asyncToGenerator(function* (pointsToLabel, pointsNotToLabel) {
    ...
    const pi = P.reduce(function (i, j) {
      return i.availableMeasure < j.availableMeasure ? i : j;
    });
    ...
    rijloop: for (let rij of R) {
      ...
      if (!Vbest || utils.compareArraysLexicographically(Vij, Vbest) < 0) {
        rbest = rij;
        Vbest = Vij;
        minimumAvailableSpace = Vij.reduce(function (i, j) {
          return Math.min(i, j);
  ...
  return function findBestRay(_x, _x2) {
    return _ref.apply(this, arguments);
  };
})();
...
let rayIntersection = (() => {
  var _ref = _asyncToGenerator(function* (pointsToLabel, pointsNotToLabel) {
    ...
    const rejectedPoints = pointsToLabel.filter(function (p) {
      return p.availableMeasure === 0;
    });
    // P in the article
    var remainingPoints = pointsToLabel.filter(function (p) {
      return p.availableMeasure > 0;
    });
    ...
    while (remainingPoints.length !== 0) {
      let bestRay = yield findBestRay.findBestRay(remainingPoints, pointsNotToLabel);
  ...
  return function rayIntersection(_x, _x2) {
    return _ref.apply(this, arguments);
  };
})();
